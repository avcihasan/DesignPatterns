...
builder.Services.AddScoped<ProcessHandler>(sp =>
{
    var excel = new ExcelProcessHandler();
    excel.SetNext(new ZipFileProcessHandler()).SetNext(new DownloadZipProcessHandler());
    return excel;
});
...
if (!userManager.Users.Any())
{
    ...
    Enumerable.Range(1, 50).ToList().ForEach(x =>
    {
        ...
        context.Categories.Add(new() { Name = $"Category {x}" });
    });
    context.SaveChanges();
}
